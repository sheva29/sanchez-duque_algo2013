...
// Set all the forces to have a single direction
void FlowField::setSingleDirection(){
    for( int y = 0; y < flowList.size(); y++){
        for( int x = 0; x < flowList[y].size(); x++){
            ofVec2f dir(1,0);
            flowList[x][y].set(dir);
...
//Set all the forces in the field based on amplitude modulation
void FlowField::setAm(){
    for( int y = 0; y < flowList.size(); y++){
        for( int x = 0; x < flowList[y].size(); x++){
           flowList[x][y].set((sin(2 * x) * sin ( 30 * x)), ( sin( 2 * y) * sin ( 30 * y)));
            
        }
    }
}

void FlowField::setModularAm(){
    for( int y = 0; y < flowList.size(); y++){
        for( int x = 0; x < flowList[y].size(); x++){
            flowList[x][y].set(2 * cos( 2 * cos(x)), 2 * cos( 2 * sin(x)));
